...
async def on_ready():
  ...
  await client.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="over your *fate..."))
...
# Export all d100 rolls in the database (debugging)
  if msg == "*fate d100dump":
    value = db["d100"]
    await message.channel.send(value)
    return
    
# main listener
  if msg.startswith("*fate "):
    comment = 0
    for i in msg: 
      if i == '*': 
        comment = comment + 1
